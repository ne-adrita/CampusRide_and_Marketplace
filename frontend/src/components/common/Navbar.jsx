...
import { FaStore, FaCar, FaEnvelope, FaUser, FaSignOutAlt, FaBars, FaTimes, FaPlus } from 'react-icons/fa';
              ...
              <>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div className="relative group">
                    ...
                    <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-dropdown border border-gray-100 py-1 hidden group-hover:block">
                      <Link to="/dashboard" className="block px-4 py-2 text-sm hover:bg-gray-50">Dashboard</Link>
                      <Link to={`/profile/${user?.id}`} className="block px-4 py-2 text-sm hover:bg-gray-50">Profile</Link>
                      <Link to="/wishlist" className="block px-4 py-2 text-sm hover:bg-gray-50">Wishlist</Link>
                      {isAdmin && (
                        <Link to="/admin" className="block px-4 py-2 text-sm hover:bg-gray-50 text-primary-600">Admin Panel</Link>
                      )}
                      <hr className="my-1" />
        ...
        {isMenuOpen && (
          <div className="md:hidden py-4 border-t border-gray-100">
            <div className="flex flex-col space-y-3">
              <Link to="/marketplace" className="flex items-center space-x-2 px-2 py-2 hover:bg-gray-50 rounded-lg">
                <FaStore size={18} /><span>Marketplace</span>
              </Link>
              <Link to="/rides" className="flex items-center space-x-2 px-2 py-2 hover:bg-gray-50 rounded-lg">
                <FaCar size={18} /><span>Rides</span>
              </Link>
              {isAuthenticated ? (
                <>
                  <Link to="/dashboard" className="flex items-center space-x-2 px-2 py-2 hover:bg-gray-50 rounded-lg">
                    <FaUser size={18} /><span>Dashboard</span>
                  </Link>
                  <Link to="/messages" className="flex items-center space-x-2 px-2 py-2 hover:bg-gray-50 rounded-lg">
